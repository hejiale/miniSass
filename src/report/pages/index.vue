<template>
  <div class="container" v-if="reportBusinessObject">
    <div
      class="backBox"
    ...
    <div class="titleBox">
      <div class="titleBox-left">
        <image
          :src="reportPublishObject.avatar"
          mode="aspectFill"
          class="titleBox-left-avatar"
        />
        <div class="titleBox-left-info">
          <span>{{ reportPublishObject.nickName || '' }}</span>
        </div>
      </div>
    </div>
    <div class="comment" v-if="reportPublishObject.content">
      {{ reportPublishObject.content }}
    </div>
    <div class="space"></div>
...
export default {
  data() {
    return {
      ...
      canDo: false,
      /*剧本封装组件业务类型*/
      reportPublishObject: null,
      reportBusinessObject: null,
    };
  },
  async onLoad() {
    this.reportBusinessObject = uni.getStorageSync('reportDetail');
    this.reportPublishObject = uni.getStorageSync('reportPublish');
    ...
    uni.removeStorageSync('reportPublishContent');
  },
  computed: {
    isImage() {
      return function (v) {
  ...
  methods: {
    ...
    async getFeedType() {
      try {
        this.reportMenu = await this.$api.getFeedType({});
      } catch (e) {}
    },
    async addReportRequest() {
      ...
      try {
        await this.$api.addUserFeedBackRecord({
          businessId: this.reportBusinessObject.id,
          feedBusiness: this.reportBusinessObject.feedBusiness,
          joinFeedBusiness: this.reportBusinessObject.joinFeedBusiness,
          content: this.remark,
