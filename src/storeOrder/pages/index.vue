      ...
      <view slot="content">
        ...
        <scroll-view scroll-y class="container-listBox" @scrolltolower="reachBottom">
          ...
          >
            ...
            <span class="container-listBox-item-date" v-if="formatDate(showTime(item))"
              >时间：{{formatDate(showTime(item))}}{{$dayjs(showTime(item)).format("HH:mm")}}</span>
            <span class="container-listBox-item-date" v-else
              >时间：{{
                $dayjs(showTime(item)).format("YYYY-MM-DD HH:mm")
              }}</span
            >
            <span class="container-listBox-item-price"
              >实付：¥{{ item.payAmount / 100 }}</span
            >
            <image src="../static/image/gather-tag.png" class="container-listBox-item-typeTag" v-if="item.productType === 'GATHERING_PRODUCT'"/>
...
export default {
  ...
  data() {
    return {
      ...
      menus: [
        { title: "全部", type: null },
        { title: "已支付", type: "PAYED" },
        { title: "已完成", type: "USED" },
        { title: "已结算", type: "SETTLEMENT" },
  ...
  computed: {
    ...
    showTime(){
      return function(item){
        return item.gatherOrderInfo && item.gatherOrderInfo.gameStartTime ? item.gatherOrderInfo.gameStartTime: item.createdTime;
      }
    }
  },
  methods: {
